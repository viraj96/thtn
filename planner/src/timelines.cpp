...
string Token::to_string() const {
    ...
    for (arg_and_type k : knowns)
        for (arg_and_type tk_var : tk_task.vars)
            if (tk_var.first == k.first) result += " " + k.second + " ";

    result += "\n\t Start Timepoint = " + start_t;
    tuple<double, double> bounds = stn.get_feasible_values(start_t);
    result += " (" + std::to_string(abs(get<0>(bounds))) + ", " +
              std::to_string(get<1>(bounds)) + ")";
...
string Auxiliary_Token::to_string() const {
    ...
    result += "\n\t Start Timepoint = " + start_t;
    tuple<double, double> bounds = stn.get_feasible_values(start_t);
    result += " (" + std::to_string(get<0>(bounds)) + ", " +
              std::to_string(get<1>(bounds)) + ")";
...
string Timeline::to_string() const {
    string result = "Timeline: ";
    result += "\n\t Id = " + id + "\n";
    result += "\t Resource = " + resource + "\n";
    result += "\t Tokens = \n";
    for (Token tk : tokens) result += "\t\t " + tk.to_string() + "\n";
...
string task_vertex::to_string() const {
...
string task_network::to_string() const {
...
void Token::set_external() { this->external = true; }

bool Token::is_external() const { return this->external; }

void Token::set_name(string _name) { this->name = _name; }

string Token::get_name() const { return this->name; }

void Token::set_resource(string _resource) { this->resource = _resource; }

string Token::get_resource() const { return this->resource; }

void Token::set_request_id(string _request_id) {
    this->request_id = _request_id;
}

string Token::get_request_id() const { return this->request_id; }

bool Token::create_timepoints(double duration) {
    int start_tp = 0, end_tp = 0;
    start_tp = stn.num_points() + 1;
    end_tp = start_tp + 1;
    this->start_t =
        this->resource + "_" + this->name + "_start" + std::to_string(start_tp);
    this->end_t =
        this->resource + "_" + this->name + "_end" + std::to_string(end_tp);

    if (duration == 0.0) duration = zero;
...
bool Token::create_timepoints(STN *stn, double duration) {
    int start_tp = 0, end_tp = 0;
    start_tp = stn->num_points() + 1;
    end_tp = start_tp + 1;
    this->start_t =
...
string Token::get_end() const { return this->end_t; }

string Token::get_start() const { return this->start_t; }

void Token::add_effects(literal eff) { this->effect_literals.insert(eff); }

void Token::add_effects(vector<literal> eff) {
    for (literal l : eff) this->effect_literals.insert(l);
}

set<literal> Token::get_effects() const { return this->effect_literals; }

void Token::add_preconditions(literal prec) {
    this->precondition_literals.insert(prec);
}

void Token::add_preconditions(vector<literal> prec) {
    for (literal l : prec) this->precondition_literals.insert(l);
}

set<literal> Token::get_preconditions() const {
    return this->precondition_literals;
}

void Token::add_knowns(arg_and_type assignment) {
    this->knowns.insert(assignment);
}

void Token::add_knowns(vector<arg_and_type> assignments) {
    for (arg_and_type a : assignments) this->knowns.insert(a);
}

void Token::change_known(arg_and_type new_known) {
    auto it = find_if(this->knowns.begin(), this->knowns.end(),
                      [&new_known](const arg_and_type k) {
...
set<arg_and_type> Token::get_knowns() const { return this->knowns; }

void Token::add_arguments(arg_and_type argument) {
    this->arguments.insert(argument);
}

void Token::add_arguments(vector<arg_and_type> arguments) {
    for (arg_and_type a : arguments) this->arguments.insert(a);
}

set<arg_and_type> Token::get_arguments() const { return this->arguments; }

string Timeline::get_id() { return this->id; }

string Timeline::get_resource() { return this->resource; }

void Timeline::add_token(Token t) { this->tokens.push_back(t); }

void Timeline::del_token(int token_loc) {
    assert(token_loc >= 0);
    assert(token_loc < int(this->tokens.size()));
    this->tokens.erase(this->tokens.begin() + token_loc);
}

void Timeline::del_tokens(pair<int, int> range) {
    assert(range.first >= 0);
    assert(range.first < int(this->tokens.size()));
    assert(range.second >= 0);
    assert(range.second < int(this->tokens.size()));
    this->tokens.erase(this->tokens.begin() + range.first,
                       this->tokens.begin() + range.second);
}

void Timeline::insert_token(Token t, Token prev, Token next) {
    int position = 0, counter = 1;
    for (auto it = this->tokens.begin(); it != this->tokens.end() - 1; it++) {
        if (*it == prev && *(it + 1) == next) position = counter;
        counter++;
    }
...
vector<Token> Timeline::get_tokens() { return this->tokens; }

void Timeline::set_id(string _id) { this->id = _id; }

void Timeline::set_resource(string _resource) { this->resource = _resource; }

void Plan::add_timeline(Timeline t) { this->timelines.push_back(t); }

vector<Timeline> Plan::get_timelines() { return this->timelines; }

Timeline *Plan::get_timelines(string id) {
    Timeline *t = nullptr;
...
Timeline *Plan::get_timelines(string id, string resource) {
    Timeline *t = nullptr;
    for (auto it = this->timelines.begin(); it != this->timelines.end(); it++)
...
Token instantiate_token(string name, string resource, string request_id,
                        double duration) {
...
Token instantiate_token(string name, string resource, STN *stn,
                        string request_id, double duration) {
    if (name == "head" || name == "tail") {
        Auxiliary_Token atk(name);
        atk.set_resource(resource);
        atk.create_timepoints();
...
Timeline instantiate_timeline(string id, string resource) {
    Timeline t = Timeline();
    t.set_id(id);
        ...
        if (sd.has_parent_sort &&
            ...
            for (string ds : sd.declared_sorts)
                for (string s : sorts[ds]) {
                    if (find(constants[ds].begin(), constants[ds].end(), s) !=
                        constants[ds].end())
                        continue;
...
task_vertex_t get_vertex(string id, task_network G) {
    task_vertex_it v = task_vertex_it(), vend = task_vertex_it();
    for (boost::tie(v, vend) = boost::vertices(G.adj_list); v != vend; v++)
        if (G.adj_list[*v].tk.get_name() == id) return *v;

    return task_vertex_t();
}

void add_sync_constraints(method m, vector<Token> tokens) {
    for (sync_constraints s : m.synchronize_constraints) {
        auto it0 = find_if(m.ps.begin(), m.ps.end(), [&s](const plan_step &p) {
            return p.id == s.task1;
        });
        auto it1 = find_if(m.ps.begin(), m.ps.end(), [&s](const plan_step &p) {
            return p.id == s.task2;
        });
        ...
        for (Token tk : tokens) {
            if (tk.get_name().substr(0, tk.get_name().find("-")) == it0->task)
                first.push_back(tk);
            else if (tk.get_name().substr(0, tk.get_name().find("-")) ==
                     it1->task)
                last.push_back(tk);
        }
        ...
        for (Token tk1 : first)
            for (Token tk2 : last) {
                ...
                if (ub == lb && ub == zero)
                    sync_name =
                        tk1.get_end() + meets_constraint + tk2.get_start();
                else
                    sync_name =
                        tk1.get_end() + before_constraint + tk2.get_start();
                constraint sync =
                    make_tuple(tk1.get_end(), tk2.get_start(), lb, ub);
                assert(stn.add_constraint(sync_name, sync));
            }
    }
}

void add_contains_constraints(method m, Token parent, vector<Token> children) {
    if (!m.ordering.size()) {
        for (Token tk : children) {
            ...
            if (mname != m.name) {
                string st_name =
                    parent.get_start() + contains_constraint + tk.get_start();
                constraint contains_st =
                    make_tuple(parent.get_start(), tk.get_start(), zero, inf);
                string et_name =
                    tk.get_end() + contains_constraint + parent.get_end();
                constraint contains_et =
                    make_tuple(tk.get_end(), parent.get_end(), zero, inf);
        ...
        for (arg_and_type o : m.ordering) {
            ...
            if (o.first != "mprec" && o.second != "mprec") {
                inner++;
                if (inner == 1) first_step = o.first;
                if (outer == (int)m.ordering.size()) last_step = o.second;
            }
        }

        auto it0 = find_if(
            m.ps.begin(), m.ps.end(),
            [&first_step](const plan_step &p) { return p.id == first_step; });
        auto it1 = find_if(
            m.ps.begin(), m.ps.end(),
            [&last_step](const plan_step &p) { return p.id == last_step; });
...
bool check_compatibility_of_argument(string arg, vector<arg_and_type> vars,
                                     set<arg_and_type> args) {
...
bool check_compatibility_of_literals(literal l, vector<arg_and_type> vars,
                                     set<arg_and_type> args) {
    bool compatible = false;
    for (string s : l.arguments) {
        bool local_compat = check_compatibility_of_argument(s, vars, args);

        if (!local_compat) break;
...
void add_literals(task_vertex_t node_t, std::shared_ptr<task_vertex> parent,
                  task_network *tn) {
    ...
    for (method par_m : methods) {
        if (par_m.name == par_m_name) {
            ...
            if (par_m.ps[0].task == node_t_name) first = true;
            if (par_m.ps[par_m.ps.size() - 2].task == node_t_name) last = true;
            ...
            for (literal p : par.tk.get_preconditions()) {
                ...
                if (p.temp_qual == AT && p.timed == START && first &&
                    compatible)
            ...
            for (arg_and_type k : par.tk.get_knowns()) {
                bool compatible = check_compatibility_of_argument(
                    k.first, par_m.vars,
                    tn->adj_list[node_t].tk.get_arguments());
                if (compatible) tn->adj_list[node_t].tk.add_knowns(k);
            }
            ...
            if (par.tk.get_knowns().size() < par.tk.get_arguments().size()) {
                ...
                for (arg_and_type arg : par.tk.get_arguments()) {
                    auto it = find_if(knowns.begin(), knowns.end(),
                                      [&arg](const arg_and_type &k) {
                                          return k.first == arg.first;
                                      });
                    if (it == knowns.end()) {
                        bool compatible = check_compatibility_of_argument(
                            arg.first, par_m.vars,
                            tn->adj_list[node_t].tk.get_arguments());
                        if (compatible) {
                            arg_and_type k =
                                make_pair(arg.first, dummy_known_name);
...
vector<Token> expand(search_vertex t, std::shared_ptr<task_vertex> parent,
                     task_network *tn, set<search_vertex> vert, request r) {
    task pt = task();
    method m = method();
    ...
    if (!t.leaf) {
        ...
        node_tk.add_effects(
            task_name_map[method_precondition_action_name + m.name].eff);
        node_tk.add_arguments(
            task_name_map[method_precondition_action_name + m.name].vars);
        node_tk.add_preconditions(
            task_name_map[method_precondition_action_name + m.name].prec);

        node = std::make_shared<task_vertex>(
            task_vertex(node_tk, node_tk.get_name(), parent));
        ...
        for (plan_step ps : m.ps) {
            if (ps.task.substr(0, method_precondition_action_name.length()) !=
                ...
                auto it = find_if(
                    vert.begin(), vert.end(), [&ps](const search_vertex &t) {
                        string task_name = t.id.substr(0, t.id.find("-"));
                        return task_name == ps.task;
                    });
                if (it != vert.end()) {
                    vector<Token> gen_subtokens =
                        expand(*it, node, tn, vert, r);
                    generated.insert(generated.end(), gen_subtokens.begin(),
                                     gen_subtokens.end());
                }
            }
        }
...
pair<task_network, vector<arg_and_type> > instantiate_task_network(
    search_vertex *root, set<search_vertex> vert, request r) {
    ...
    for (boost::tie(v, vend) = boost::vertices(tn.adj_list); v != vend; v++) {
        if (tn.adj_list[*v].parent == nullptr) {
            ...
            string st_name =
                "cz" + cz_constraint + tn.adj_list[*v].tk.get_start();
            constraint cz_st =
                make_tuple("cz", tn.adj_list[*v].tk.get_start(), rel_time, inf);

            string et_name =
                "cz" + cz_constraint + tn.adj_list[*v].tk.get_end();
            constraint cz_et =
                make_tuple("cz", tn.adj_list[*v].tk.get_end(), zero, due_date);
        ...
        if (tk.get_arguments().size() != tk.get_knowns().size())
            for (arg_and_type arg : tk.get_arguments()) {
                ...
                if (it == knowns.end()) open.push_back(arg);
            }
    }
...
task_network assign_open_variables(map<string, arg_and_type> var_assign_map,
                                   task_network tree) {
        ...
        for (pair<string, arg_and_type> var : var_assign_map) {
            ...
            if (arguments.size() != knowns.size()) {
                for (arg_and_type arg : arguments)
                    if (arg == var.second) {
                        arg_and_type var_assign =
                            make_pair(arg.first, var.first);
            ...
            // Case where the open variable was pushed to its child
                ...
                for (arg_and_type k : knowns)
                    if (k.second == dummy_known_name &&
                        k.first == var.second.first) {
