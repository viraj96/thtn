...
string slot::to_string() const {
    ...
    for (string dep_end : dependent_ends) result += dep_end + "  ";
    result += "\n";
    return result;
}

string primitive_solution::to_string() const {
    string result = "Primitive Solution: \n";
    result += "\t Primitive Token = " + primitive_token.to_string() + "\n";
    result += "\t Slots = \n";
    for (slot s : token_slots) result += "\t\t" + s.to_string() + "\n";
    result += "\n";
    return result;
}

string tasknetwork_solution::to_string() const {
...
bool operator==(const slot &lhs, const slot &rhs) {
    ...
    return tie(l_tk_start, l_tk_end, l_prev_start, l_prev_end, l_next_start,
               l_next_end, lhs.tl_id) == tie(r_tk_start, r_tk_end, r_prev_start,
                                             r_prev_end, r_next_start,
                                             r_next_end, rhs.tl_id);
}

bool operator!=(const slot &lhs, const slot &rhs) { return !(lhs == rhs); }
...
string evaluate(string arg, set<arg_and_type> *knowns) {
    ...
    for (pair<string, set<string>> c : constants)
        for (string a : c.second)
            if (a == arg) arg_value = a;

    for (arg_and_type at : (*knowns))
        if (at.first == arg) arg_value = at.second;
...
bool check_functional_predicate(literal *precondition,
    ...
    for (string arg : precondition->arguments) {
        ...
        if (arg.find(".") != string::npos) arg_and_attr = get_arg_and_attr(arg);

        if (arg_and_attr == arg_and_type()) {
            for (arg_and_type var : *(knowns))
                if (arg == var.first) args.push_back(var.second);

        } else {
            for (arg_and_type var : (*knowns))
                if (arg_and_attr.first == var.first)
                    for (const arg_and_type &v :
                         (*current_state)[var.second].attribute_states.vars)
...
bool check_equal_predicate(literal *precondition, set<arg_and_type> *knowns,
    ...
    if (arg1_has_attr != string::npos) arg1_n_attr = get_arg_and_attr(arg1);
    if (arg2_has_attr != string::npos) arg2_n_attr = get_arg_and_attr(arg2);
    ...
    if (arg1_has_attr == string::npos && arg2_has_attr == string::npos) {
        ...
        string value2 = evaluate(arg2, knowns);
        arg_and_type arg1_n_attr_value =
            evaluate(arg1_n_attr, knowns, current_state);
        assert(arg1_n_attr_value != arg_and_type());
        lit_pass = (arg1_n_attr_value.second == value2);

    } else if (arg1_has_attr == string::npos && arg2_has_attr != string::npos) {
        string value1 = evaluate(arg1, knowns);
        arg_and_type arg2_n_attr_value =
            evaluate(arg2_n_attr, knowns, current_state);
        ...
        if (lit->predicate == precondition->predicate) {
            ...
            for (string s : precondition->arguments) {
                ...
                if (s.find(".") != string::npos) {
                    ...
                    arg_and_type arg_n_attr_value =
                        evaluate(arg_n_attr, knowns, current_state);
                    ...
                    if (lit->args[arg_num] != value) arg_pass = false;
                }
...
bool check_precondition(literal *precondition, set<arg_and_type> *knowns,
    ...
    for (ground_literal lit : (*init)) {
        ...
        if (lit_pass) return true;
    }

    return false;
}

arg_and_type get_arg_and_attr(string argument) {
...
void update_object_state(object_state *obj, Token *prev) {
    ...
    // If the previous token was the head token then you only need to extract the initial state
        ...
        if (eff.predicate == dummy_equal_literal) {
            ...
            if (arg1_n_attr != arg_and_type())
                for (auto it = obj->attribute_states.vars.begin();
                     it != obj->attribute_states.vars.end(); it++)
                    if (it->first == arg1_n_attr.second) it->second = arg2;
        }
}
...
Token gen_token(arg_and_type argument, Token *causal_token, STN *stn) {
    ...
    for (arg_and_type arg : causal_token->get_arguments()) {
        if (causal_token->get_name() == "grasp" && arg.second == "robot") {
            for (arg_and_type k : causal_token->get_knowns())
                if (k.first == arg.first) name = k.second;

        } else if (causal_token->get_name() == "release" &&
                   arg.second == "location") {
            for (arg_and_type k : causal_token->get_knowns())
                if (k.first == arg.first) name = k.second;

        } else if (causal_token->get_name() == "rail_move" &&
                   arg.second == "robot") {
            for (arg_and_type k : causal_token->get_knowns())
                if (k.first == arg.first) name = k.second;
        }
    ...
    Token tk = instantiate_token(name, argument.second, stn,
                                 causal_token->get_request_id(), 0.0);

    for (arg_and_type k : causal_token->get_knowns()) tk.add_knowns(k);
    for (arg_and_type arg : causal_token->get_arguments())
        tk.add_arguments(arg);
    for (literal eff : causal_token->get_effects()) tk.add_effects(eff);
...
set<string> get_literal_args_and_types(literal *lit,
    ...
    for (string arg : lit->arguments) {
        ...
        if (arg_and_attr != arg_and_type()) arg_to_check = arg_and_attr.first;
    ...
    for (task pt : primitive_tasks) {
        for (literal eff : pt.eff)
            if (eff.predicate == precondition->predicate &&
                ...
                for (arg_and_type v : pt.vars) vars.insert(v);
                set<string> eff_arg_types =
                    get_literal_args_and_types(&eff, &vars);
                for (string prec_type : (*prec_arg_types)) {
                    bool found = false;
                    string prec_type_copy = prec_type,
                           prec_param_type = prec_type, prec_attr_type = "";
                    if (prec_type_copy.find("-") != string::npos) {
                        prec_param_type =
                            prec_type_copy.substr(0, prec_type_copy.find("-"));
                        prec_type_copy.erase(0, prec_type_copy.find("-") + 1);
                        prec_attr_type =
                            prec_type_copy.substr(0, prec_type_copy.find("-"));
                    }
                    for (string eff_type : eff_arg_types) {
                        string eff_type_copy = eff_type,
                               eff_param_type = eff_type, eff_attr_type = "";
                        if (eff_type_copy.find("-") != string::npos) {
                            eff_param_type = eff_type_copy.substr(
                                0, eff_type_copy.find("-"));
                            eff_type_copy.erase(0, eff_type_copy.find("-") + 1);
                            eff_attr_type = eff_type_copy.substr(
                                0, eff_type_copy.find("-"));
                        }
...
pair<set<arg_and_type>, set<arg_and_type>> get_satisfying_knowns_and_args(
    ...
    set<string> *prec_arg_types, bool functional) {
    /* Arguments:
    ...
    if (functional) {
        ...
        for (method m : methods)
            if (satisfying_task->name.substr(
                    method_precondition_action_name.length()) == m.name)
                    ...
                    for (arg_and_type k : (*knowns))
                        if (k.first == arg.first) {
                            satisfying_knowns.insert(
                                make_pair(s_arg.first, k.second));
                            assigned[s_arg.first] = true;
                            break;
                        }
                if (assigned[s_arg.first]) break;
            }
    ...
    for (arg_and_type s_arg : satisfying_arguments) {
        ...
        if (!assigned[s_arg.first]) {
            for (literal prec : satisfying_task->prec) {
                ...
                if (find(prec.arguments.begin(), prec.arguments.end(),
                         s_arg.first) != prec.arguments.end())
                    for (ground_literal lit : init) {
                        // Check the list of constant predicates that match
                        // the precondition signature
                        if (prec.positive == lit.positive &&
                            prec.predicate == lit.predicate &&
                            ...
                            if (!violate) {
                                satisfying_knowns.insert(make_pair(
                                    s_arg.first, lit.args[s_arg_pos]));
                                assigned[s_arg.first] = true;
                                break;
                            }
                        }
...
bool add_meets_constraint(Token *first, Token *second, STN *stn) {
    string meets_name =
        first->get_end() + meets_constraint + second->get_start();
    constraint meets =
        make_tuple(first->get_end(), second->get_start(), zero, zero);
    bool succ = stn->add_constraint(meets_name, meets);
...
bool add_contains_constraint(Token *first, Token *second, STN *stn,
                             bool submit) {
    string st_name =
        first->get_start() + contains_constraint + second->get_start();
    constraint contains_st =
        make_tuple(first->get_start(), second->get_start(), zero, zero);

    string et_name = second->get_end() + contains_constraint + first->get_end();
    constraint contains_et =
        make_tuple(second->get_end(), first->get_end(), zero, zero);
...
tuple<bool, bool, bool> del_and_add_sequencing_constraint(Token *prev,
    ...
    string c1_name =
        prev->get_end() + sequencing_constraint + curr->get_start();
    constraint c1 = make_tuple(prev->get_end(), curr->get_start(), zero, inf);

    string c2_name =
        curr->get_end() + sequencing_constraint + next->get_start();
    ...
    if (submit) {
        ...
        if (!succ0) return make_tuple(succ0, false, false);
...
pair<bool, vector<slot>> satisfy_precondition(literal *precondition,
    ...
    set<string> prec_arg_types =
        get_literal_args_and_types(precondition, &arguments);

    pair<bool, task> potential_task =
        find_satisfying_task(precondition, &prec_arg_types);

    if (!potential_task.first) return make_pair(false, vector<slot>());
    ...
    pair<set<arg_and_type>, set<arg_and_type>> knowns_and_args =
        get_satisfying_knowns_and_args(&satisfying_task, &arguments, &knowns,
                                       current_state, &prec_arg_types,
                                       functional);
    ...
    if (!satisfying_task.artificial && !functional) {
        ...
        Token tk = instantiate_token(satisfying_task.name, "robot", stn,
        ...
        for (arg_and_type arg : satisfying_arguments) tk.add_arguments(arg);
        for (arg_and_type k : satisfying_knowns) tk.add_knowns(k);

        bool succ1 = true;
        if (prev->get_name() != "head" &&
            prev->get_request_id() == tk.get_request_id())
        ...
        for (method m : methods)
            if (m.name == satisfying_task.name.substr(
        ...
        for (string arg : satisfying_method.atargs)
            for (arg_and_type sk : satisfying_knowns)
                if (arg == sk.first) args.push_back(sk.second);

        vector<pair<task, var_declaration>> tasks_to_do =
            satisfying_method.func_method(args, current_state);
        if (tasks_to_do.size() == 0) return make_pair(false, vector<slot>());

        vector<Token> satisfying_tokens = vector<Token>();
        for (unsigned int i = 0; i < tasks_to_do.size(); i++) {
            Token tk = instantiate_token(tasks_to_do[i].first.name, "robot",
                                         stn, failing_tk->get_request_id(),
            ...
            for (literal prec : satisfying_task.prec) {
                if (prec.timed == timed_type::START && i == 0)
                    ...
                    tk.add_preconditions(prec);
                else if (prec.timed == timed_type::END &&
                         i == tasks_to_do.size() - 1)
            ...
            for (literal eff : satisfying_task.eff) {
                if (eff.timed == timed_type::START && i == 0)
                    ...
                    tk.add_effects(eff);
                else if (eff.timed == timed_type::END &&
                         i == tasks_to_do.size() - 1)
                    tk.add_effects(eff);
            }
        ...
        for (auto it = satisfying_tokens.begin();
             it != satisfying_tokens.end() - 1; it++) {
            Token curr = *it;
            Token next = *(it + 1);
        ...
        if (failing_timeline == new_timeline) {
            ...
            bool succ2 = add_meets_constraint(
                &satisfying_tokens[satisfying_tokens.size() - 1], failing_tk,
                stn);
        ...
        for (Token tk : satisfying_tokens) {
            pair<bool, vector<slot>> ret =
                schedule_token(&tk, explored, p, stn);
            ...
            satisfying_tokens_slots.insert(satisfying_tokens_slots.end(),
                                           new_slots.begin(), new_slots.end());
        }
...
bool check_temporal_bounds(slot *to_explore, STN *stn) {
    ...
    constraint meets =
        stn->get_constraint(to_explore->prev.get_end() + meets_constraint +
                            to_explore->next.get_start());
    if (get<0>(meets) != "" && get<1>(meets) != "") return false;
    ...
    constraint duration =
        stn->get_constraint(to_explore->tk.get_start() + duration_constraint +
                            to_explore->tk.get_end());

    assert(get<0>(duration) != "");
    if (to_explore->prev.get_name() != "head" &&
        to_explore->next.get_name() != "tail") {
        if (get<3>(duration) >
            get<1>(next_start_bounds) - abs(get<0>(prev_end_bounds)))
            return false;
    }
...
pair<bool, vector<slot>> schedule_token(Token *tk, vector<slot> *explored,
    ...
    for (arg_and_type arg : other_resources)
        other_resource_tokens.push_back(gen_token(arg, tk, stn));
            ...
            while (!scheduled) {
                ...
                if (!scheduled) {
                    ...
                    for (pair<string, constraint> post : post_stn_constraints)
                        if (pre_stn_constraints.find(post.first) ==
                            pre_stn_constraints.end()) {
                            assert(stn->del_constraint(post.first));
                        }
                    for (pair<string, constraint> pre : pre_stn_constraints)
                        if (post_stn_constraints.find(pre.first) ==
                            post_stn_constraints.end()) {
        ...
        if (!scheduled) {
            for (int i = 0; i < leaf_id; i++)
                for (slot s : (*solution)[i].token_slots) {
                    if (s.tk.get_start() == leafs[i].tk.get_start()) continue;
                    stn.del_timepoint(s.tk.get_start());
                    stn.del_timepoint(s.tk.get_end());
                }
    ...
    for (int i = 0; i < (int)slot_to_commit.solution.size(); i++) {
        ...
        for (slot s : leaf_slots) {
            if (s.tk.get_resource() == "robot") {
                ...
                for (task t : primitive_tasks)
                    if (s.tk.get_name() == t.name) main_dur = t.duration;

                string dur_name =
                    s.tk.get_start() + duration_constraint + s.tk.get_end();

                constraint dur = make_tuple(s.tk.get_start(), s.tk.get_end(),

                                            main_dur, main_dur);
                assert(stn.add_constraint(dur_name, dur));

                for (string dep_meets : s.dependent_ends) {
                    map<string, constraint> current_stn_constraints =
                        stn.get_constraints();
                    string match_target =
                        dependent_meets_constraint + dep_meets;
                    for (pair<string, constraint> curr :
                         current_stn_constraints)
                        if (strstr(curr.first.c_str(), match_target.c_str()))

                            assert(stn.del_constraint(curr.first));
                    string meets_name =
                        s.tk.get_end() + dependent_meets_constraint + dep_meets;
                    constraint meets =
                        make_tuple(s.tk.get_end(), dep_meets, zero, inf);
            ...
            if (s.tk.get_name() != main_tk.get_name()) {
                ...
                constraint dur =
                    make_tuple(s.tk.get_start(), s.tk.get_end(), zero, inf);
...
pair<bool, Plan> patch_plan(Plan p, Token *failing_tk,
    ...
    double earliest_start_time =
        abs(get<0>(stn.get_feasible_values(failing_tk->get_start())));
    ...
    // time
    for (Timeline t : tls) {
        if (t.get_resource() != "rail_block" &&
            t.get_id() != failing_timeline.get_id()) {
            for (Token tk : t.get_tokens()) {
                if (abs(get<0>(stn.get_feasible_values(tk.get_start()))) <
                    earliest_start_time) {
    ...
    for (i = failing_tk_index; failing_timeline_tks[i] != original_failing_tk;
         i++) {
        if (failing_timeline_tks[i].is_external()) {
            task ftk_task = task();
            for (task t : primitive_tasks)
                            ...
                            for (int j = 0; j < (int)rt_tokens.size(); j++) {
                                ...
                                if (get<0>(contains) != "") {
                                    ...
                                    if (j < (int)rt->get_tokens().size()) {
                                        string seq_name =
                                            rt_tokens[j - 1].get_end() +
                                            sequencing_constraint +
                                            rt_tokens[j + 1].get_start();
                                        constraint seq = make_tuple(
                                            rt_tokens[j - 1].get_end(),
                                            rt_tokens[j + 1].get_start(), zero,
                                            inf);
                                        assert(
                                            stn.add_constraint(seq_name, seq));
                                    }
                                    break;
                                }
                            }
    ...
    failing_timeline_tks =
        p.get_timelines(failing_timeline.get_id())->get_tokens();
    if (abs(failing_tk_index - i) > 0 &&
        failing_tk_index < (int)failing_timeline_tks.size()) {
        ...
        constraint seq = make_tuple(
            failing_timeline_tks[failing_tk_index - 1].get_end(),
            failing_timeline_tks[failing_tk_index].get_start(), zero, inf);
    ...
    for (literal prec : original_failing_tk.get_preconditions()) {
        ...
        if (find(tk_task.prec.begin(), tk_task.prec.end(), prec) !=
            ...
            if (!succ) {
                ...
                pair<bool, vector<slot>> return_slots = satisfy_precondition(
                    &prec, &original_failing_tk,
                    &failing_timeline_tks[failing_tk_index - 1], &current_state,
                    &plan_copy, &stn, &empty_explored);
            ...
            for (Token tk : tks) {
                ...
                for (arg_and_type arg : tk.get_arguments())
                    if (is_resource(arg.second) &&
                        ...
                        for (arg_and_type k : tk.get_knowns())
                            if (k.first == arg.first) {
                                if (arg.second != "robot" &&
                                    arg.second != "rail_block") {
                                    resources.push_back(
                                        make_pair(k.second, arg.second));
                                    for (map<string, var_declaration> cs :
                                         csorts[arg.second])
                                        for (pair<string, var_declaration> m :
                                             cs)
                                            if (m.first == k.second) {
                                                object_state obj =
                                                    object_state();
                                                obj.object_name = m.first;
                                                obj.attribute_states = m.second;
                                                current_state.insert(make_pair(
                                                    obj.object_name, obj));
                                            }
                                }
                            }
                ...
                for (arg_and_type o_r : resources) {
                    ...
                    for (Token rtk : rt->get_tokens()) {
                        string contains_co = tk.get_start() +
                                             contains_constraint +
                                             rtk.get_start();
                ...
                for (literal prec : tk.get_preconditions()) {
                    if (find(tk_task.prec.begin(), tk_task.prec.end(), prec) !=
                        ...
                        bool succ = check_precondition(&prec, &tk_knowns,
                                                       &current_state, init);
                        if (!succ) return make_pair(false, tk);
                    }
                }
                ...
                if (tk.get_name() == "rail_move") {
                    ...
                    for (arg_and_type a : tk.get_arguments()) {
                        if (a.second == "rail_block")
                            for (arg_and_type k : tk.get_knowns())
                                if (k.first == a.first) sink.id = k.second;
                    }
                    for (arg_and_type var :
                         current_state[t.get_id()].attribute_states.vars)
                        for (sort_definition sd : sort_definitions)
                            if (find(sd.declared_sorts.begin(),
                                     sd.declared_sorts.end(),
                                     "robot") != sd.declared_sorts.end())
                                for (arg_and_type v : sd.vars.vars)
                ...
                for (arg_and_type o_r : resources) {
                    ...
                    for (Token rtk : rt->get_tokens()) {
                        ...
                        if (rtk == rt_pivots[counter]) break;
                    }
...
pq find_feasible_slots(task_network tree, Plan p, int attempts, string metric) {
    ...
    for (boost::tie(vi, vi_end) = boost::vertices(tree.adj_list); vi != vi_end;
         vi++)
    ...
    for (int plan_id = 0; plan_id < attempts; plan_id++) {
        ...
        if (success) {
            vector<tuple<int, int, slot>> main =
                vector<tuple<int, int, slot>>();
            vector<tuple<int, int, slot>> dependents =
                vector<tuple<int, int, slot>>();
            ...
            for (int i = 0; i < (int)solution.size(); i++)
                for (int j = 0; j < (int)solution[i].token_slots.size(); j++)
                    if (solution[i].token_slots[j].tk.get_resource() == "robot")
                        main.push_back(
                            make_tuple(i, j, solution[i].token_slots[j]));
                    else
            ...
            for (tuple<int, int, slot> os : main) {
                string dep_meets_substr =
                    get<2>(os).tk.get_end() + dependent_meets_constraint;
                for (pair<string, constraint> post_stn_c : post_stn_constraints)
                    if (strstr(post_stn_c.first.c_str(),
                               dep_meets_substr.c_str())) {
                        solution[get<0>(os)]
                            .token_slots[get<1>(os)]
                            .dependent_ends.push_back(
...
bool is_resource(string type) {
    for (sort_definition sd : sort_definitions)
        if (find(sd.declared_sorts.begin(), sd.declared_sorts.end(), type) !=
            sd.declared_sorts.end())
            if (sd.has_parent_sort &&
                sd.parent_sort == "discrete_reusable_resource")
                return true;
    return false;
}
...
map<string, arg_and_type> find_assignments(string cart_prod,
    ...
    map<string, arg_and_type> arg_to_assignment_map =
...
bool clear(vector<string> arguments, world_state *current_state) {
    ...
    if (source.id == sink.id) return true;
    ...
    for (sort_definition sd : sort_definitions)
        if (find(sd.declared_sorts.begin(), sd.declared_sorts.end(), "robot") !=
            sd.declared_sorts.end())
            for (arg_and_type var : sd.vars.vars)
                if (var.second == "rail_block") rail_attr = var.first.substr(1);
    ...
    for (map<string, var_declaration> cs : csorts["robot"])
        for (pair<string, var_declaration> m : cs)
            if ((*current_state).find(m.first) != (*current_state).end())
                for (arg_and_type attr :
                     (*current_state)[m.first].attribute_states.vars)
                ...
                for (pair<string, var_declaration> m : cs)
                    if (m.first == ws.first)
                        for (arg_and_type v : ws.second.attribute_states.vars)
                            if (v.first == "block")
                                occupied.push_back(
                                    get_vertex(v.second, rail_network));
...
vector<pair<task, var_declaration>> clear_and_move(vector<string> arguments,
    ...
    vector<pair<task, var_declaration>> result =
            ...
            for (map<string, var_declaration> cs : csorts["robot"])
                for (pair<string, var_declaration> m : cs)
                    if (m.first == ws.first)
                        for (arg_and_type v : ws.second.attribute_states.vars)
                            if (v.first == "block")
                                occupied.push_back(make_pair(
                                    ws.first,
                                    get_vertex(v.second, rail_network)));

        } else {
            for (arg_and_type var : ws.second.attribute_states.vars)
                if (var.first == "block") source.id = var.second;
        }
    }

    if (source.id == sink.id) return result;
    ...
    vertex_t offending_source = vertex_t(),
             smallest_offending_source = vertex_t();
    for (vector<vertex_t> path : paths) {
        free = true;
        vector<pair<string, vertex_t>>::iterator it =
            vector<pair<string, vertex_t>>::iterator();
        for (vertex_t v : path) {
            it = find_if(occupied.begin(), occupied.end(),
                         [&v](const pair<string, vertex_t> &p) {
                             return p.second == v;
                         });
            if (it != occupied.end()) {
                free = false;
        ...
        if (!free) {
            ...
            for (boost::tie(v, vend) = boost::vertices(rail_network.adj_list);
                 v != vend; v++) {
                if (find(path.begin(), path.end(), *v) == path.end()) {
                    ...
                    sort(offending_paths.begin(), offending_paths.end(),
                         [](const vector<vertex_t> &a,
                            const vector<vertex_t> &b) {
                             return a.size() < b.size();
                         });

                    for (vector<vertex_t> off_path : offending_paths) {
                        if (find(off_path.begin(), off_path.end(), s) ==
                            off_path.end()) {
    ...
    for (task t : primitive_tasks)
        if (!t.artificial && t.name == "rail_move") {
            for (vertex_t p : smallest_offending_path) {
                if (p == smallest_offending_source) continue;
                var_declaration vardecl = var_declaration();
                for (arg_and_type var : t.vars) {
                    if (var.second == "robot")
                        vardecl.vars.push_back(
                            make_pair(var.first, smallest_offending_robot));
                    else if (var.second == "rail_block")
                        vardecl.vars.push_back(
                            make_pair(var.first, rail_network.adj_list[p].id));
                }
...
vector<pair<task, var_declaration>> reachable_location(
    ...
    for (auto it = blocks.begin(); it != blocks.end(); it++)
        if (it.value()[0].get<double>() < location.y &&
            it.value()[1].get<double>() >= location.y)
            sink.id = it.key();
